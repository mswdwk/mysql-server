...
/* sm3 sha1 style interface  */
void compute_sm3_hash(uint8 *digest,unsigned char *input,size_t ilen)
{
    sm3_context ctx;

    sm3_starts( &ctx );
    sm3_update( &ctx, input, (int)ilen );
    sm3_finish( &ctx, digest );

    memset( &ctx, 0, sizeof( sm3_context ) );
}

void compute_sm3_hash_multi(uint8 *digest,unsigned char *buf1,int len1,unsigned char* buf2,int len2)
{
    sm3_context ctx;

    sm3_starts( &ctx );
    sm3_update( &ctx, buf1, len1 );
    sm3_update( &ctx, buf2, len2 );
    sm3_finish( &ctx, digest );
